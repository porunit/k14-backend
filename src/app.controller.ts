...
async function extractTextContent(page) {
  ...
  const LIST_ITEMS_SELECTOR = "[data-testid=\"result-list-container\"] a[data-testid^=\"result-listing-\"]";
  ...
  const listItems = listItemsElements.map((index, element) => {
    const url = $(element).attr("href");
    const detailsElement = $(element).find("[data-testid=\"listing-details-attributes\"]");
    ...
    const imgElements = $(element).find("[data-testid^=\"result-listing-image-\"]");
    const imgUrls = imgElements.map((i, imgElement) => imgElement.attribs["src"]).get();
...
export class AppController {
  ...
  async getHello(
    ...
    const { yearFrom, yearTo, priceFrom, priceTo, mileageFrom, mileageTo, page, model, brand }: {
      page?: string;
      brand?: string;
      yearFrom?: string;
      yearTo?: string;
    ...
    if(!selectedBrand){
      return {
        error: {
          ...
          details: optionElements.map(r => ({...r, url: `http://localhost:3001/?${[...Object.entries(query), ['brand', r.label]].map(([k, v]) => `${k}=${v}`).join("&")}`  }))
        }
      };
    }

    const modelsResult = await this._page.evaluate((selectedBrand) => {
      return fetch(`https://m.mobile.de/consumer/api/search/reference-data/models/${selectedBrand.value}`).then(res => res.json());
    }, selectedBrand);

    console.log(modelsResult);

    const modelsNameValueMap = new Map(modelsResult.data.filter(r => !!r.value).map(i => [i.label, i.value]));
      ...
      return {
        error: {
          ...
          details: modelsResult.data.map(r => {
            const queryWithoutModel = Object.entries(query).filter(([key]) => key !== 'model');

            if(!r.items){
              return {...r, url: `http://localhost:3001/?${[...queryWithoutModel, ['model', r.label]].map(([k, v]) => `${k}=${v}`).join("&")}`  };
            } else {
              return {...r, items: r.items.map(rr => ({...rr, url: `http://localhost:3001/?${[...queryWithoutModel, ['model', rr.label]].map(([k, v]) => `${k}=${v}`).join("&")}`  }))  };
            }
          })
        }
      };
    ...
    const searchString = Object.entries(queryParamsMap).filter(([key, value]) => !!value).map(([key, value]) => `${key}=${value}`).join("&");
    ...
    await this._page
      .waitForSelector("[data-testid=\"result-list-container\"]");
